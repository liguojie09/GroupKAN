...
def parse_args():
    ...
    parser.add_argument('--dataseed', default=6142, type=int,
                        help='')
    
    # model
    parser.add_argument('--arch', '-a', metavar='ARCH', default='KANet')
    ...
    parser.add_argument('--input_h', default=256, type=int,
                        help='image height')
    parser.add_argument('--active_kan_group', default=16, type=int,
                        help='image height')
    parser.add_argument('--group_kan_num', default=16, type=int,
                        help='image height')
...
def main():
    ...
    # create model
    model = KANet.__dict__[config['arch']](config['num_classes'], config['input_channels'], config['deep_supervision'],
                                           embed_dims=config['input_list'], no_kan=config['no_kan'],
                                           active_kan_group =config['active_kan_group'], group_kan_num=config['group_kan_num'])
    # model = UNet(in_channels=config['input_channels'], num_classes=config['num_classes'],channels=[16, 32, 128, 160, 256])
    # model = UNet(in_channels=3, out_channels=1, base_dim=64, attn_heads=4)
    model = model.cuda()
    ...
    shutil.copy2('KANet.py', f'{output_dir}/{exp_name}/')
